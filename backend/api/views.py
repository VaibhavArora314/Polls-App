...
class VoteViewSet(GenericViewSet, CreateModelMixin, UpdateModelMixin, RetrieveModelMixin):
    ...
    def retrieve(self, request, *args, **kwargs):
        instance = get_object_or_404(
            Votes, poll=kwargs['pk'], user=request.user)
    ...
    def create(self, request, *args, **kwargs):
        ...
        if Votes.objects.filter(user=request.user, poll=serializer.validated_data['poll']).exists():
            return Response({"Multiple votes are not allowed. Edit your previous vote in case u want to change vote."}, status=status.HTTP_403_FORBIDDEN)
    ...
    def perform_update(self, serializer):
        ...
        old_option = Votes.objects.get(
            poll=self.kwargs['pk'], user=self.request.user).option.id
